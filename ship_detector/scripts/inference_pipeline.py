...
class InferencePipeline:
    ...
    def _load_vit_model(self):
        ...
        if self.config.vit_checkpoint.endswith(".ckpt"):
            checkpoint = torch.load(
                self.config.vit_checkpoint, map_location="cpu", weights_only=False
            )
    ...
    def _load_unet_model(self):
        ...
        if self.config.unet_checkpoint.endswith(".ckpt"):
            checkpoint = torch.load(
                self.config.unet_checkpoint, map_location="cpu", weights_only=False
            )
            model.load_state_dict(checkpoint["state_dict"])
        else:
            model.load_state_dict(
    ...
    def tile_image(self, image: np.ndarray) -> Tuple[List[Dict], Dict]:
        ...
        logger.info(
            f"Tiling image: {w}x{h} into {self.config.patch_size}x{self.config.patch_size} patches"
        )
        ...
        if (
            w % stride != 0
            and h % stride != 0
            and w > self.config.patch_size
            and h > self.config.patch_size
        ):
            row = h - self.config.patch_size
            col = w - self.config.patch_size
            patch_data = image[
                row : row + self.config.patch_size, col : col + self.config.patch_size
            ]
    ...
    def segment_patches(
        self, patches: List[Dict], ship_indices: List[int]
    ) -> Dict[int, np.ndarray]:
    ...
    def extract_ship_crops(
        self, image: np.ndarray, mask: np.ndarray, padding: int = 10
    ) -> List[Dict]:
        ...
        for label_id in range(1, num_labels):
            ...
            contours, _ = cv2.findContours(
                ship_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
            if not contours:
                continue
    ...
    def create_json_output(
        self, mask: np.ndarray, image_path: str, metadata: Dict
    ) -> Dict:
        ...
        for label_id in range(1, num_labels):
            ...
            # Find contours
            contours, _ = cv2.findContours(
                ship_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
                    ...
                    for file in files:
                        ...
                        try:
                            self.s3_client.upload_file(
                                file_path, self.config.s3_bucket, s3_key
                            )
    ...
    def process_image(self, image_path: str, output_dir: str) -> Dict:
        ...
        if self.config.save_patch_predictions:
            ...
            patch_results.to_csv(
                os.path.join(output_dir, f"{image_name}_patches.csv"), index=False
            )
        ...
        full_mask = self.stitch_masks(
            patches, segmentation_masks, (metadata["height"], metadata["width"])
        )
        ...
        outputs = self.save_outputs(
            output_dir, image_name, image, full_mask, json_data, overlay, ship_crops
        )

        # Processing stats
        num_ships = (
            len(ship_crops) if ship_crops else cv2.connectedComponents(full_mask)[0] - 1
        )
        ...
        logger.info(
            f"Completed {image_name} in {metadata['processing_time']:.2f} seconds"
        )
    ...
    def process_batch(self, image_paths: List[str], output_dir: str) -> List[Dict]:
        ...
        for image_path in tqdm(image_paths, desc="Processing images"):
            try:
                ...
                results.append(
                    {"image": Path(image_path).stem, "status": "error", "error": str(e)}
                )
...
def run_inference(inference_cfg_path: str, output_dir: str):
    config = load_config(config_path=inference_cfg_path)

    if not config.get("image", None) and not config.get("batch", None):
        click.echo("Error: Please provide either --image or --batch")
        return

    # Create configuration
    pipeline_config = PipelineConfig(
        vit_checkpoint=config["vit_checkpoint"],
        vit_config=config["vit_config"],
        unet_checkpoint=config["unet_checkpoint"],
        ...
        output_format=config["formats"],
        save_patch_predictions=config["save_patches"],
    )
    ...
    if config["image"]:
        ...
        stats = pipeline.process_image([config["image"]], output_dir)
        ...
        for ext in image_extensions:
            image_paths.extend(Path(config["batch"]).glob(f"*{ext}"))
            image_paths.extend(Path(config["batch"]).glob(f"*{ext.upper()}"))

        if not image_paths:
            click.echo(f"No images found in {config['batch']}")
            return
