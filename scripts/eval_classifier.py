...
def plot_confusion_matrix(
    ...
    plt.title('Confusion Matrix - Ship Detection', fontsize=14, fontweight='bold')
    ...
    for i in range(2):
        for j in range(2):
            ...
            text = f'\n({percentage:.1f}%)'
            plt.text(j + 0.5, i + 0.7, text, ha='center', va='center', fontsize=10, color='gray')
...
def plot_roc_curve(
    ...
    plt.title('Receiver Operating Characteristic - Ship Detection', fontsize=14, fontweight='bold')
...
def plot_prediction_distribution(
    ...
    plt.hist(no_ship_probs, bins=bins, alpha=0.6, label='No Ship (True)', color='blue', density=True)
    plt.hist(ship_probs, bins=bins, alpha=0.6, label='Ship (True)', color='red', density=True)
...
def save_error_analysis(
    ...
    # Save top errors
    top_errors = errors_df.head(top_k)[['patch_path', 'true_label', 'predicted_label', 'prediction_prob', 'confidence']]
...
def main(
    ...
    print(f"Ship patches: {eval_df['has_ship'].sum()}, ({eval_df['has_ship'].mean()*100:.1f}%)")
    ...
    plot_prediction_distribution(labels, probs, os.path.join(output_dir, 'prediction_dist.png'))
    
    # Error analysis
    error_stats = save_error_analysis(eval_df, labels, preds, probs, output_dir)
    ...
    with open(report_file, 'w') as f:
        ...
        f.write(f"Ship samples: {int(labels.sum())} ({labels.mean()*100:.1f}%)\n")
        f.write(f"No-ship samples: {int(len(labels) - labels.sum())} ({(1 - labels.mean())*100:.1f}%)\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate ship detection classifier")
    parser.add_argument('--checkpoint', type=str, required=True, help='Path to model checkpoint')
    parser.add_argument('--config', type=str, required=True, help='Path to config YAML file')
    parser.add_argument('--manifest', type=str, required=True, help='Path to manifest CSV file')
    parser.add_argument('--output-dir', type=str, default='./evaluation/vit', help='Path to output directory')
    parser.add_argument('--split', type=str, default='val', choices=['val', 'test'], help='Data split to evaluate')
