...
if __name__ == '__main__':
    ...
    # Fix image shape to align with model input dimension
    
    class ShipPreservingDataset(Dataset):
    """Enhanced dataset with ship-preserving preprocessing"""
    
    def __init__(self, manifest_df, config, transform=None, is_training=True):
        self.manifest = manifest_df.reset_index(drop=True)
        self.config = config
        self.transform = transform
        self.is_training = is_training
        self.preprocessing_method = config['model'].get('preprocessing_method', 'adaptive')
    ...
    def multiscale_resize_with_context(self, image):
        ...
        detail_view = cv2.resize(center_crop, (112, 112), interpolation=cv2.INTER_AREA)
        
        # Combine views
        combined = np.zeros((224, 224, 3), dtype=image.dtype)
        combined[:112, :112] = global_view
        combined[:112, 112:] = detail_view
        combined[112:, :] = cv2.resize(image, (224, 112), interpolation=cv2.INTER_AREA)
    ...
    def adaptive_resize_preserve_ships(self, image):
        ...
        if self.config['data'].get('apply_sharpening', False):
            ...
            resized = cv2.addWeighted(resized, 1 - strength, sharpened, strength, 0)
    ...
    def __getitem__(self, idx):
        ...
        # Load original image (assuming 768x768)
        ...
        if image.shape[:2] != (768, 768):
            # If not 768x768, resize to 768x768 first to maintain consistency
...
def get_enhanced_transforms(config):
    """Get enhanced transforms for ship detection"""
    aug_config = config['augmentation']
    ...
    if aug_config.get('color_jitter', False):
        ...
        train_transforms.append(
            transforms.ColorJitter(
                ...
                contrast=contrast,
                saturation=0.1,  # Low saturation changes for satellite imagery
...
def create_enhanced_data_loaders(manifest_path: str, config: dict):
    """Create enhanced data loaders with ship-preserving preprocessing"""
    ...
    if config['data'].get('min_ship_pixel_ratio'):
        # This would require additional preprocessing to calculate ship sizes
        pass
    ...
    print(f"  - With ships: {train_df['has_ship'].sum()} ({train_df['has_ship'].mean()*100:.1f}%)")
    print(f"Validation samples: {len(val_df)}")
    print(f"  - With ships: {val_df['has_ship'].sum()} ({val_df['has_ship'].mean()*100:.1f}%)")
    
    # Get transforms
    train_transforms, val_transforms = get_enhanced_transforms(config)
    
    # Create datasets with enhanced preprocessing
    train_dataset = ShipPreservingDataset(train_df, config, transform=train_transforms, is_training=True)
    val_dataset = ShipPreservingDataset(val_df, config, transform=val_transforms, is_training=False)
    
    # Create data loaders with enhanced sampling
    if config['training'].get('use_weighted_sampler', False):
        train_labels = train_df['has_ship'].values
        class_counts = np.bincount(train_labels)
        
        # Enhanced weighting for small ship detection
